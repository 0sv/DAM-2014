$(function() {
    ...
    var getTweets = function(data) {
        ...
        $.each(data, function(index) {
            ...
            db.transaction(function(tx) {
                tx.executeSql('INSERT INTO tweets (id, user, date, text) VALUES (?, ?, ?, ?)', [tweet.id, tweet.from_user, tweet.time, tweet.text]);
            });
        });
    };
    ...
    var getUsers = function(data) {
        ...
        $.each(data, function(index) {
            var user = data[index];
            db.transaction(function(tx) {
                tx.executeSql('INSERT INTO users (id, name) VALUES (?, ?)', [user.id, user.name]);
            });
        });
    };
    ...
    var addTweet = function(tweet1) {
        ...
        db.transaction(function(tx) {
            ...
            tx.executeSql('INSERT INTO tweets (id, user, date, text) VALUES (?, ?, ?, ?)', [tweet1.id, tweet1.from_user, time / 1000, tweet1.text]);
        });
